...
import { db } from "@/db";
import { likesTable, tweetsTable } from "@/db/schema";
...
export async function POST(request: NextRequest) {
  ...
  const { handle, content, startTime, endTime, replyToTweetId } = data as PostTweetRequest;
  try {
    ...
    const tweetID = await db
      ...
      .onConflictDoNothing()
      .execute();
    return NextResponse.json({ body: tweetID }, { status: 200 });
  } catch (error) {
    console.log(error);
